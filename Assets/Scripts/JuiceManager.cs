...
public class JuiceManager : MonoBehaviour
{
    ...
    public void PlayExplosionVFX(Vector3 position, Color color)
    {
        if (explosionVfxPrefab != null)
        {
            // 在指定位置生成特效
            GameObject vfxObj = Instantiate(explosionVfxPrefab, position, Quaternion.identity);
            if (vfxObj.TryGetComponent<ParticleSystem>(out var ps))
            {
                var main = ps.main;
                main.startColor = color;
            }
            // 播放完成后自动销毁（为了性能，假设特效持续时间不超过 1.5 秒）
            Destroy(vfxObj, 1.5f);
        }
    }
}